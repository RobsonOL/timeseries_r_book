...
Contudo, objetos `ts` podem não ser tão conveniente de trabalhar quanto o uso de objetos `data.frame` ou `tibble`. Mais recentemente, O pacote `timetk` possui uma série de funções convenientes para converter objetos `ts` para outros formatos. A função `tk_tbl` converte o objeto `ts` para um objeto `tibble` (objeto equivalente a um dataframe). Já a função `tk_ts` faz o caminho inverso.  Neste caso, temos uma coluna para as informações de data e outra coluna com as observações de passageiros.
...
A partir do objeto `dados_passageiros` é possível produzir o mesmo gráfico utilizando o pacote `timetk` e sua função `plot_time_series()`, bastando informar qual a coluna com dados de data e qual a coluna com a variável dependente (figura \@ref(fig:passageiros-ggplot)).
...
Ao descrever séries temporais, usamos palavras como **tendência**, **sazonalidade** e **padrão cíclico**. Antes disso, podemos observar séries sem padrão definido.
...
Para responder esta questão, podemos criar uma variável relativa a chuva do ano anterior e compara-la com a chuva no ano corrente utilizando a função `lag()`. A tabela \@ref(tab:tab-chuva) mostra essa comparação.
...
A figura \@ref(fig:figura-diabetes) mostra uma série de vendas de drogas contra diabetes nos EUA. A série possui um comportamento sazonal bastante marcante.
...
Como temos uma série mensal de vendas de drogas ao longo dos anos, podemos verificar o comportamento a sazonalidade dentro dos anos, trimestres e meses. O pacote `timetk` oferece uma série de funções para diagnóstico de padrões nas séries temporais. 

A figura \@ref(fig:seasonal_timetk) cada série anual (o período sazonal) de maneira independente. 
...
Uma série de tempo possui três padrões básicos: tendência, sazonalidade e ciclo. Uma série de técnicas podem ser utilizadas para separar estes componentes individuais.  Geralmente, os componentes de ciclo e tendência são exibidos em conjunto (e chamados apenas de tendência), de modo que uma série temporal pode ser vista como composta de três componentes: um componente de ciclo-tendência, um componente de sazonalidade e o resto. 
...
A decomposição STL é implementada no R com a função `stat::stl()`. O pacote  `timetk` fornece uma versão conveniente desta função com `plot_stl_diagnostics()`. Vamos aplicar esta função aos dados de equipamentos elétricos produzidos na zona do Euro (\@ref(fig:electrical)).

A função `plot_stl_diagnostics()` tem dois parâmetros principais:
...
O valor padrão destes dois parâmetros é `"auto`, que permite que a função calcule automaticamente o período de sazonalidade e a janela de tendência. A figura \ref(fig:decomp-stl) mostra o resultado da decomposição STL.
...
```
Na figura vemos cinco séries: (1) série observada; (2) o componente sazonal; (3) o ciclo tendência; (4) o resto; e a série ajustada sazonalmente.
